...
class SwitchButton(Button):
  def switch_commit_context_views(self, from_view, to_view):
    ...
    visualblame.get_view_by_id(to_view).process_event_result([commit_context_data])
    # except:


class BlameCommitSwitchButton(SwitchButton, EventWidget):
  def on_press(self):
    self.switch_commit_context_views("blame_commit_context", "diff_commit_context")
...
class DiffCommitSwitchButton(SwitchButton, EventWidget):
  ...
  def on_press(self):
    try:
      self.switch_commit_context_views("diff_commit_context", "blame_commit_context")
