...
class Blame(GitModuleBase):
    ...
    def _get_blame_lines(self):
        ...
        try:
            blame_obj = self._repo.blame(self.file_path,
                                         newest_commit=str(self.newest_commit))
        except KeyError:
            logging.error("Blame: blame failed, no such path '" +
                          self.file_path + "' in " + str(self.newest_commit))
            sys.exit()
