...
class Diff(GitModuleBase):
    ...
    def execute(self):
        try:
            diff = self._repo.diff(str(self.commit_id) + "^",
                                   str(self.commit_id), context_lines=0)
            # Merge the diffs that are caused by a renamed file
            diff.find_similar()
        except KeyError:
            # First commit has no parent, manually get its added lines
            first_commit_result = self._get_first_commit_diff_data(
                self.commit_id)
        ...
        for commit_file in diff:
            ...
            hunks = self._create_diff_hunk_list(commit_new_file_lines,
                                                commit_file.hunks)

            diff_data[commit_file_path_rel] = self._init_file_diff(
                hunks, commit_file.line_stats)
    ...
    def _create_diff_hunk_list(self, file_lines, diff_hunks):
        ...
        for hunk in diff_hunks:
            ...
            if hunk_first_lineno > last_lineno:
                commit_file_hunks.append(self._init_hunk(
                    " ", file_lines[last_lineno-1:hunk_first_lineno-1]))
            ...
            for line in hunk.lines:
                ...
                try:
                    if commit_file_hunks[-1].origin == line.origin:
                        ...
                        commit_file_hunks.append(
                            self._init_hunk(line.origin, [line_content]))
                except IndexError:
                    # In the case the line is the first, there won't be a hunk
                    # in the results yet, thus create it here
                    commit_file_hunks.append(self._init_hunk(line.origin,
                                                             [line_content]))

        # If there are lines in the file after the last hunk, add them here
        if last_lineno < len(file_lines):
            commit_file_hunks.append(self._init_hunk(
                " ", file_lines[last_lineno-1:len(file_lines)]))
    ...
    def _get_patch_new_file_lines(self, patch):
        ...
        if commit_file_lines is not None:
        ...
        while len(trees) > 0:
            ...
            for entry in tree:
                if entry.type == "blob":
                    ...
                    diff_data[prepend + entry.name] = self._init_file_diff(
                        [self._init_hunk("+", lines)], (0, len(lines), 0))
                elif entry.type == "tree":
                    prepend += entry.name
                    trees.append(TreeData(prepend + entry.name + "/",
                                 self._repo.get(str(entry.id))))
